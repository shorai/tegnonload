...
public class Sensor {
    ...
    public Sensor(String vals) {
         ...
         device = Device.findId(Integer.parseInt(strs[i++]));
         ...
         try {
            ...
            lineId = -1; 
            // LineID is often NULL logger.log(Level.SEVERE,"LineID:" + strs[i-1] + " " + exc.getMessage(), exc);
         }
         try {
         netId = Integer.parseInt(strs[i++]);
         } catch(Exception exc) {
            netId = -1; 
            // netId is often NULL logger.log(Level.SEVERE,"NetID:" + strs[i-1] + " " + exc.getMessage(), exc);
         }
         try{
